...
2. **Host-enforced mode**: an optional local adapter (`tools/enforce.py`) that injects the canonical protocol into a host-controlled model invocation, validates returned Signal structure, retries repairs, and fails closed instead of releasing a non-compliant response.
